...
int VAL_ASI_MODE   = 0, VAL_ASI_FRONT = 1, VAL_ASI_REAR   = 2;
int CMD_ASI_MANUAL = 1, CMD_ASI_AUTO  = 2, CMD_ASI_FRONT  = 3, CMD_ASI_REAR  = 4;
...
int asi_pin_discr = 14;
int asi_pin_front = 15;
int asi_pin_rear  = 16;

boolean flag_button_x_down = true;  // Not compleated
boolean flag_button_y_down = true;
boolean flag_button_lb_down= true;
...
void make_updown( int* p_gamepad, boolean* asi )
{
    // LB(DISC) Flag Check and Do
    /*if (p_gamepad[ CMD_ASI_DISCR ] && flag_button_lb_down) {
        asi[ VAL_ASI_DISCR ] = !asi[ VAL_ASI_DISCR ];
        flag_button_lb_down = false;
    }
    else if (!p_gamepad[ CMD_ASI_DISCR ]) {
        flag_button_lb_down = true;
    }*/

    // Mode Flag Check and Do
    if (p_gamepad[ CMD_ASI_MANUAL ] ) {
        asi[ VAL_ASI_MODE ] = true;
    }
    else if (p_gamepad[ CMD_ASI_AUTO ] ) {
        asi[ VAL_ASI_MODE ] = false;
    }

    // FRONT Flag Check and Do
    if (p_gamepad[ CMD_ASI_FRONT ] && flag_button_x_down) {
        asi[ VAL_ASI_FRONT ] = !asi[ VAL_ASI_FRONT ];
        flag_button_x_down = false;
    }
    else if (!p_gamepad[ CMD_ASI_FRONT ]) {
        flag_button_x_down = true;
    }

    // REAR Flag Check
    if (p_gamepad[ CMD_ASI_REAR ] && flag_button_y_down) {
        asi[ VAL_ASI_REAR ] = !asi[ VAL_ASI_REAR ];
        flag_button_y_down = false;
    }
    else if (!p_gamepad[ CMD_ASI_REAR ]) {
        flag_button_y_down = true;
    }
}

void send_updown( boolean* asi )
{
    digitalWrite( asi_pin_discr, asi[ VAL_ASI_MODE ] );
    digitalWrite( asi_pin_front, asi[ VAL_ASI_FRONT ] );
    digitalWrite( asi_pin_rear , asi[ VAL_ASI_REAR  ] );
}
...
void loop()
{
    ...
    static boolean asi[] = { false, false, false };     // { Mode, Front, Rear }
    int pwm_val[] = { 0, 0, 0, 0 };
    ...
    else if (p_gamepad[0] == updown )
    {
        make_updown( p_gamepad, asi );
        send_updown( asi );
    }
    

    //Serial.println(p_gamepad[0]);
